...
public class StudentController {
    ...
    public Result<Map<String, Object>> getDashboard(Authentication auth) {
        Long userId = (Long) auth.getPrincipal();
        return Result.success(studentService.getDashboardSummary(userId));
    }

    @PostMapping("/data-integration/authorize")
    public Result<Void> saveDataAuthorization(Authentication auth, @RequestBody DataAuthorizationDTO dto) {
        Long userId = (Long) auth.getPrincipal();
        studentService.saveDataAuthorization(userId, dto);
        return Result.success();
    }

    @GetMapping("/data-integration/records")
    public Result<List<DataIntegrationRecord>> getDataIntegrationRecords(Authentication auth) {
        Long userId = (Long) auth.getPrincipal();
        return Result.success(studentService.getMyIntegrationRecords(userId));
    }
}
