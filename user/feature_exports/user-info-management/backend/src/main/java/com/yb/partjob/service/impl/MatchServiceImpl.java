...
public class MatchServiceImpl implements IMatchService {
    ...
    public void computeMatchScores(Long studentId) {
        ...
        // Avoid generating misleading "default" scores when the student has not
        // provided enough preference data for a real comparison yet.
        if (!hasComparableSignals(student)) {
            matchScoreRepository.deleteByStudentId(studentId);
            return;
        }
    ...
    private boolean hasComparableSignals(StudentProfile student) {
        return !parseSkills(student.getSkills()).isEmpty()
                || student.getExpectedSalaryMin() != null
                || student.getExpectedSalaryMax() != null
                || (student.getExpectedLocation() != null && !student.getExpectedLocation().isBlank())
                || (student.getAvailableSchedule() != null && !student.getAvailableSchedule().isBlank());
    }
}
