...
public class AdminController {
    ...
    @GetMapping("/data-integration/overview")
    public Result<Map<String, Object>> getDataIntegrationOverview() {
        return Result.success(adminService.getDataIntegrationOverview());
    }

    @GetMapping("/data-integration/records")
    public Result<Page<DataIntegrationRecord>> getDataIntegrationRecords(
            @RequestParam(required = false) String targetType,
            @RequestParam(required = false) String verificationStatus,
            @RequestParam(required = false) String sourceType,
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int size) {
        return Result.success(adminService.getDataIntegrationRecords(targetType, verificationStatus, sourceType, page, size));
    }

    @PutMapping("/data-integration/records/{recordId}/review")
    public Result<Void> reviewDataIntegrationRecord(@PathVariable Long recordId,
            @RequestParam String status,
            @RequestParam(required = false) String remark) {
        adminService.reviewDataIntegrationRecord(recordId, status, remark);
        return Result.success();
    }

    @GetMapping("/data-integration/rules")
    public Result<Map<String, String>> getDataIntegrationRules() {
        return Result.success(adminService.getDataIntegrationRules());
    }

    @PutMapping("/data-integration/rules")
    public Result<Void> saveDataIntegrationRules(@RequestBody Map<String, String> rules) {
        adminService.saveDataIntegrationRules(rules);
        return Result.success();
    }
}
