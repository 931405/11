...
public class JobController {
    ...
    public Result<Page<JobVO>> searchJobs(@RequestParam(required = false) String keyword,
        ...
        return Result.success(jobService.searchJobs(keyword, categoryId, location, jobType, enterpriseId, page, size, userId));
    }

    @GetMapping("/{jobId}")
    public Result<JobVO> getJobDetail(@PathVariable Long jobId, Authentication auth) {
        Long userId = auth == null ? null : (Long) auth.getPrincipal();
        return Result.success(jobService.getJobDetail(jobId, userId));
    }
