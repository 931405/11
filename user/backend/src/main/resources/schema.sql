...
-- 14. Data Integration Record Table
CREATE TABLE IF NOT EXISTS data_integration_record (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    source_type VARCHAR(50) NOT NULL COMMENT 'Integration source type',
    source_name VARCHAR(100) NOT NULL COMMENT 'Integration source name',
    target_type VARCHAR(20) NOT NULL COMMENT 'Target domain type',
    target_id BIGINT NOT NULL COMMENT 'Target record ID',
    submitted_by_user_id BIGINT COMMENT 'Submitting user ID',
    authorization_status VARCHAR(20) NOT NULL DEFAULT 'NOT_REQUIRED' COMMENT 'NOT_REQUIRED, AUTHORIZED, REVOKED',
    verification_status VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT 'PENDING, VERIFIED, REJECTED',
    quality_score DECIMAL(5,2) COMMENT 'Normalized quality score',
    raw_payload TEXT COMMENT 'Original payload snapshot',
    normalized_payload TEXT COMMENT 'Normalized payload snapshot',
    rule_version VARCHAR(50) COMMENT 'Applied rule version',
    remark VARCHAR(500) COMMENT 'Review or sync remark',
    last_verified_at DATETIME COMMENT 'Latest verification time',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
) ENGINE=InnoDB COMMENT='Normalized integration records across student, enterprise, and job data';
